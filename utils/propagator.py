...
from odak.learn.tools import zero_pad, crop_center, generate_2d_gaussian , circular_binary_mask
...
def circular_binary_mask(px, py, r, cx=0, cy=0):
    ...
    cx           : int, optional
                   Center x-coordinate of the circle. If None, defaults to center (0).
    cy           : int, optional
                   Center y-coordinate of the circle. If None, defaults to center (0).
...
class propagator():
    ...
    def __init__(
        ...
        use_cuda_blasm          : bool
                                  Use CUDA bandlimited angular spectrum method.
        device                  : torch.device
        ...
        self.use_cuda_blasm = use_cuda_blasm
        self.use_cuda_blasm_gaussian_version = False
    ...
    def __call__(self, input_field, channel_id, depth_id):
        ...
        if self.use_cuda_blasm and self.propagation_type == 'Bandlimited Angular Spectrum':
            output_field = BandlimitedPropagation(
                input_field,
                self.wavelengths[channel_id],
                self.pixel_pitch,
                distance.item() if isinstance(distance, torch.Tensor) else distance,
                [self.resolution[0] * self.resolution_factor, self.resolution[1] * self.resolution_factor],
                use_cuda=True,
                use_cuda_gaussian_version = self.use_cuda_blasm_gaussian_version,
                aperture_size=self.aperture_size if self.aperture_size is not None else -1.0
            )
            return output_field
    ...
    def _reconstruct_impl(self, hologram_phases, amplitude=None, get_complex=False):
        ...
        if not self.use_cuda_blasm or self.propagation_type != 'Bandlimited Angular Spectrum':
        ...
        for frame_id in range(self.number_of_frames):
            ...
            for depth_id in range(self.number_of_depth_layers):
                for channel_id in range(self.number_of_channels):
                    ...
                    if get_complex:
                        complex_fields[frame_id, depth_id, channel_id] = output_field
                        reconstructions[frame_id, depth_id, channel_id] = calculate_amplitude(output_field) ** 2
                    else:
                        reconstructions[frame_id, depth_id, channel_id] = calculate_amplitude(output_field) ** 2
        
        if get_complex:
            return reconstructions, complex_fields
        else:
